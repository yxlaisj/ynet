...
/**
 * 存在的问题：read和write都要抛出线程，且需要在循环中不断key.cancel()避免重复调用（这也是一步系统调用）
 */
public class MultipleThreadsSelectorTest {
                ...
                while (selector.select(50) > 0) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (key.isAcceptable()) {
                            ...
                            key.cancel();  // 反复的系统调用 epoll_ctl(delete)
                            ...
                            //要明白：什么时候写？不是依赖send-queue是不是有空间
                            //1，你准备好要写什么了，这是第一步
    ...
    public static void main(String[] args) {
        MultipleThreadsSelectorTest service = new MultipleThreadsSelectorTest();
        service.start();
    }
}
